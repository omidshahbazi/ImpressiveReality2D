...
public:
	...
	b2_float32 ReportFixture(	b2Fixture* fixture, const b2Vec2& point,
		const b2Vec2& normal, b2_float32 fraction)
	{
		...
		if (userData)
		{
			b2_int32 index = *(b2_int32*)userData;
			if (index == 0)
			{
				// filter
				return -1.0f;
...
public:
	...
	RayCast()
	{
		...
		{
			b2_float32 w = 1.0f;
			b2_float32 b = w / (2.0f + b2Sqrt(2.0f));
			b2_float32 s = b2Sqrt(2.0f) * b;
	...
	void Create(b2_int32 index)
	{
		...
		b2_float32 x = RandomFloat(-10.0f, 10.0f);
		b2_float32 y = RandomFloat(0.0f, 20.0f);
		bd.position.Set(x, y);
	...
	void DestroyBody()
	{
		for (b2_int32 i = 0; i < e_maxBodies; ++i)
		{
			if (m_bodies[i] != NULL)
			{
	...
	void Step(Settings* settings)
	{
		...
		else if (m_mode == e_multiple)
		{
			...
			for (b2_int32 i = 0; i < callback.m_count; ++i)
			{
	...
	b2_int32 m_bodyIndex;
	b2Body* m_bodies[e_maxBodies];
	b2_int32 m_userData[e_maxBodies];
