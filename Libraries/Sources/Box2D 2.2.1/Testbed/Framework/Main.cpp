...
namespace
{
	b2_int32 testIndex = 0;
	b2_int32 testSelection = 0;
	b2_int32 testCount = 0;
	TestEntry* entry;
	Test* test;
	Settings settings;
	b2_int32 width = 640;
	b2_int32 height = 480;
	b2_int32 framePeriod = 16;
	b2_int32 mainWindow;
	float settingsHz = 60.0;
	GLUI *glui;
	b2_float32 viewZoom = 1.0f;
	int tx, ty, tw, th;
	bool rMouseDown;
	b2Vec2 lastp;
}

static void Resize(b2_int32 w, b2_int32 h)
{
	width = w;
	height = h;
	...
	glLoadIdentity();
	b2_float32 ratio = b2_float32(tw) / b2_float32(th);
...
static b2Vec2 ConvertScreenToWorld(b2_int32 x, b2_int32 y)
{
	b2_float32 u = x / b2_float32(tw);
	b2_float32 v = (th - y) / b2_float32(th);
...
static void Mouse(b2_int32 button, b2_int32 state, b2_int32 x, b2_int32 y)
{
...
static void MouseMotion(b2_int32 x, b2_int32 y)
{
	b2Vec2 p = ConvertScreenToWorld(x, y);
	test->MouseMove(p);
