...
void Test::PreSolve(b2Contact* contact, const b2Manifold* oldManifold)
{
	...
	for (b2_int32 i = 0; i < manifold->pointCount && m_pointCount < k_maxContactPoints; ++i)
	{
...
{
	b2_float32 timeStep = settings->hz > 0.0f ? 1.0f / settings->hz : b2_float32(0.0f);
	...
	{
		b2_int32 bodyCount = m_world->GetBodyCount();
		b2_int32 contactCount = m_world->GetContactCount();
		b2_int32 jointCount = m_world->GetJointCount();
		m_debugDraw.DrawString(5, m_textLine, "bodies/contacts/joints = %d/%d/%d", bodyCount, contactCount, jointCount);
		m_textLine += 15;

		b2_int32 proxyCount = m_world->GetProxyCount();
		b2_int32 height = m_world->GetTreeHeight();
		b2_int32 balance = m_world->GetTreeBalance();
		b2_float32 quality = m_world->GetTreeQuality();
	...
	if (settings->drawProfile)
	{
		...
		if (m_stepCount > 0)
		{
			b2_float32 scale = 1.0f / m_stepCount;
	...
	{
		//const b2_float32 k_impulseScale = 0.1f;
		const b2_float32 k_axisScale = 0.3f;

		for (b2_int32 i = 0; i < m_pointCount; ++i)
		{
