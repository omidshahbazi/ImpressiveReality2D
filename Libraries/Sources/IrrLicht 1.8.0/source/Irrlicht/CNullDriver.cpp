...
//	DriverAttributes->addInt("AntiAlias", 0);
	...
	for (u32 i=0; i<video::MATERIAL_MAX_TEXTURES; ++i)
	{
		...
		InitMaterial2D.TextureLayer[i].Texture = NULL;
	}
...
ITexture* CNullDriver::getTexture(const io::path& filename)
{
	...
	//texture = findTexture(absolutePath);
	//if (texture)
	//	return texture;
...
//! Draws a 3d polygon.
void CNullDriver::draw3DPolygon(const core::vector3d<f32> &center, double radius, u32 vertexCount, core::vector3d<f32> &normal, video::SColor color, bool drawNormal)
{
	...
	core::vector3d<f32> v1 = core::vector3d<f32>(0, normal.Z, -normal.Y);
	core::vector3d<f32> v2 = core::vector3d<f32>(normal.Z, 0, -normal.X);

	//scale vector to unit length (dividing by (1 − x2)1/2) where x is the 0 component of the vector, in the normal.
	if (v1 != core::vector3d<f32>(0,0,0))
	{   
	...
	for (u32 i = 0; i <= vertexCount; i++)
	{
		...
		core::vector3d<f32> vec1 = center + radius * cos(2 * core::PI64 / vertexCount * i) * v1 + radius * sin(2 * core::PI64 / vertexCount * i) * v2;
   
		core::vector3d<f32> vec2 = center + radius * cos(2 * core::PI64 / vertexCount * (i + 1)) * v1 + radius * sin(2 * core::PI64 / vertexCount * (i + 1)) * v2;

		draw3DLine(vec1, vec2, color);
	}

	if (drawNormal)
		draw3DLine(center, center + (normal * 10) );
}
