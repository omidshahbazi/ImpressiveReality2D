...
bool activateJoysticks(core::array<SJoystickInfo> & joystickInfo)
{
	...
	joystickInfo.floatlocate(numberOfJoysticks);
	ActiveJoysticks.floatlocate(numberOfJoysticks);
	...
	ExternalWindow(false), Win32CursorControl(0), JoyControl(0)
{
	...
	if (!CreationParams.WindowId && CreationParams.DriverType != video::EDT_NULL)
	{
		...
		const s32 floatWidth = clientSize.right - clientSize.left;
		const s32 floatHeight = clientSize.bottom - clientSize.top;

		s32 windowLeft = (GetSystemMetrics(SM_CXSCREEN) - floatWidth) / 2;
		s32 windowTop = (GetSystemMetrics(SM_CYSCREEN) - floatHeight) / 2;
		...
		HWnd = CreateWindow( ClassName, __TEXT(""), style, windowLeft, windowTop,
					floatWidth, floatHeight, NULL, NULL, hInstance, NULL);
		CreationParams.WindowId = HWnd;
//		CreationParams.WindowSize.Width = floatWidth;
//		CreationParams.WindowSize.Height = floatHeight;

		ShowWindow(HWnd, SW_SHOWNORMAL);
		UpdateWindow(HWnd);

		// fix ugly ATI driver bugs. Thanks to ariaci
		MoveWindow(HWnd, windowLeft, windowTop, floatWidth, floatHeight, TRUE);

		// make sure everything gets updated to the float sizes
		Resized = true;
	}
...
void CIrrDeviceWin32::setResizable(bool resize)
{
	...
	const s32 windowLeft = (GetSystemMetrics(SM_CXSCREEN) - floatWidth) / 2;
	const s32 windowTop = (GetSystemMetrics(SM_CYSCREEN) - floatHeight) / 2;

	SetWindowPos(HWnd, HWND_TOP, windowLeft, windowTop, floatWidth, floatHeight,
